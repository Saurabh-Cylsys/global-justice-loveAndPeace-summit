...
declare var AOS: any;
@Component({
  selector: 'app-first-partner',
  templateUrl: './first-partner.component.html',
  styleUrls: ['./first-partner.component.css']
})


export class FirstPartnerComponent {
  ...
  this.form.controls['urn_no'].setValue(this.form.controls['urn_no'].value.trim());
    ...
    if (!emailValue && !mobileValue && !urnValue) {
      ...
      this.SpeakerService.checkEmailAndMobile_1(signUpData).subscribe((res: any) => {
        if (res.success) {
